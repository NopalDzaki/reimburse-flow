...
export default function TicketDashboardPage() {
  ...
  const getPriorityBadge = (p: string) => {
    switch (p.toLowerCase()) {
      case "urgent":
      case "high":
        return <Badge variant="destructive">{p}</Badge>;
      case "medium":
      ...
      case "open":
        return (
          <span className="flex items-center gap-1.5 text-sm font-medium text-info">
            <span className="h-2 w-2 rounded-full bg-info"></span>Open
          </span>
        );
      case "in_review":
        return (
          <span className="flex items-center gap-1.5 text-sm font-medium text-warning">
            <span className="h-2 w-2 rounded-full bg-warning"></span>In Progress
          </span>
        );
      case "resolved":
        return (
          <span className="flex items-center gap-1.5 text-sm font-medium text-success">
            <span className="h-2 w-2 rounded-full bg-success"></span>Resolved
          </span>
        );
      default:
        return (
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-muted-foreground">
            Manage and resolve internal system reports.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-xl border border-border/50 bg-card p-6 shadow-sm flex flex-col justify-center">
          ...
          <div className="flex items-baseline gap-2">
            <h2 className="text-4xl font-heading font-bold text-info">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Button
                          variant="ghost"
                          size="sm"
                          className="hidden group-hover:flex items-center gap-1.5 text-primary"
                          asChild
                        >
