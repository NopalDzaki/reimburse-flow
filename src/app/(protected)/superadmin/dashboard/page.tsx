...
export default function SuperadminDashboardPage() {
  ...
  const urgentTickets = reports.filter(
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <div className="rounded-xl border border-border/50 bg-card p-6 shadow-sm flex flex-col justify-center">
          ...
          <p className="text-sm font-medium text-success mt-2 flex items-center gap-1">
            <span className="flex h-2.5 w-2.5 rounded-full bg-success/20 items-center justify-center">
                  ...
                  {users.slice(0, 5).map((u) => (
                    <tr key={u.id} className="hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-1 text-xs rounded-sm font-semibold uppercase ${u.role === "admin" ? "bg-info/10 text-info" : u.role === "superadmin" ? "bg-primary/10 text-primary" : u.role === "finance" ? "bg-warning/10 text-warning" : "bg-muted text-muted-foreground"}`}
                        >
                          {u.role}
                        </span>
                      </td>
          ...
          <div className="flex-1 relative space-y-6 overflow-y-auto pr-2 before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-border before:via-border/50 before:to-transparent">
            {auditLogs.map((log) => (
              <div key={log.id} className="relative flex items-start gap-4">
                <div
                  className={`absolute left-0 mt-1 flex h-5 w-5 items-center justify-center rounded-full bg-background ring-4 ring-background z-10 ${log.color}`}
                >
                  <div
                    className={`h-2.5 w-2.5 rounded-full ${log.bg} border border-current`}
                  ></div>
                </div>
                <div className="pl-8 flex flex-col w-full">
                  ...
                  <span
                    className={`text-sm font-semibold font-heading mb-0.5 ${log.color}`}
                  >
                    {log.type}
                  </span>
