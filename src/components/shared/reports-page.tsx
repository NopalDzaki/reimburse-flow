  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={role === "superadmin" ? "Global view of support requests." : "Issues and tickets you have submitted to Superadmin."}
        actions={
          role !== "superadmin" ? (
      ...
      {viewableReports.length === 0 ? (
        <EmptyState 
          ...
          description={role === "superadmin" ? "The support queue is empty." : "Submit a support ticket and it will appear here."} 
          action={role !== "superadmin" ? <Link href={createHref}><Button>Create First Report</Button></Link> : undefined} 
        />
      ) : (
            ...
            <div key={r.id} className="rounded-xl border border-border/50 bg-card p-4 sm:p-5 shadow-sm flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button asChild variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-primary hover:text-primary">
                  <Link href={getDetailHref(r.id)}><Eye className="h-4 w-4" /></Link>
                </Button>
              </div>
            </div>
          ))}
