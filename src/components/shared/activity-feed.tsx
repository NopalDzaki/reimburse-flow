...
export function ActivityFeed({ activities, maxItems = 10 }: { activities: ActivityLog[], maxItems?: number }) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        {activities.slice(0, maxItems).map((item) => {
          const IconEl = (iconMap as Record<string, typeof Bell>)[item.type] ?? Bell;
          return (
            <div key={item.id} className="flex gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground font-medium">{getRelativeTime(item.createdAt)}</p>
              </div>
            </div>
          );
        })}
