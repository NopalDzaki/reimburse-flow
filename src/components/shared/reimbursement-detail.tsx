...
export function ReimbursementDetail({ reimbursement, actions, footer, showPaymentProof = true }: ReimbursementDetailProps) {
  return (
        ...
        <CardHeader className="pb-4">
          <div className="flex flex-col gap-2">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="secondary" className="rounded-full">{reimbursement.id}</Badge>
              <StatusBadge status={reimbursement.status} />
              {reimbursement.reviewedBy && (
                <Badge variant="outline" className="rounded-full text-xs">Reviewed by {reimbursement.reviewedBy}</Badge>
              )}
            </div>
            <CardTitle className="text-2xl">{reimbursement.title}</CardTitle>
            <p className="text-sm text-muted-foreground max-w-2xl leading-relaxed">{reimbursement.description}</p>
          </div>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <InfoRow label="Amount" value={formatCurrencyIDR(reimbursement.amount)} highlight />
            <InfoRow label="Transaction Date" value={formatDateID(reimbursement.transactionDate)} />
            <InfoRow label="Category" value={reimbursement.category} />
            <InfoRow label="Submitted" value={formatDateID(reimbursement.submittedAt)} />
            <InfoRow label="Bank" value={`${reimbursement.bankName} • ${reimbursement.accountNumber}`} />
            <InfoRow label="Account Holder" value={reimbursement.accountHolderName} />
          </div>
          ...
          {actions && (
            <div className="rounded-xl border border-border/50 bg-muted/30 p-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Approve, reject, or mark as paid with notes.</p>
              </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            ...
            {showPaymentProof && <Attachment label="Payment Proof" href={reimbursement.paymentProofImage} />}
              ...
              <div className="pt-3 border-t border-border/40 space-y-2">
                {reimbursement.adminNote && (
                  <div>
                    ...
                    <p className="text-sm text-foreground leading-relaxed">{reimbursement.adminNote}</p>
                  </div>
                )}
                {reimbursement.financeNote && (
                  <div>
                    <p className="text-xs uppercase tracking-wider text-muted-foreground mb-1">Finance Note</p>
                    <p className="text-sm text-foreground leading-relaxed">{reimbursement.financeNote}</p>
                  </div>
                )}
              </div>
            )}
...
function InfoRow({ label, value, highlight }: { label: string; value?: string; highlight?: boolean }) {
  return (
    <div className="rounded-lg border border-border/40 bg-card/50 p-4 shadow-sm">
      ...
      <p className={`text-sm sm:text-base font-semibold mt-1 ${highlight ? "text-foreground" : "text-foreground/90"}`}>{value || "-"}</p>
    </div>
  )
}
