...
export function SharedReportDetail({
  id,
  role,
  backHref,
}: SharedReportDetailProps) {
        ...
        <div className="space-y-4 pt-4 border-t border-border/40">
          ...
          {report.replies.map((msg) => {
            if (msg.isSystem)
              return (
                ...
                >
                  ...
                  <span className="shrink-0 px-3 py-1 rounded-full border border-border/50 bg-muted/50">
                    {msg.body}
                  </span>
                  <div className="flex-1 h-px bg-border/50" />
                </div>
            ...
            return (
              <div
                key={msg.id}
                className={`flex gap-3 ${isMe ? "relative flex-row" : "flex-row-reverse"}`}
              >
                ...
                <div
                  className={`flex-1 max-w-[80%] ${isMe ? "" : "flex flex-col items-end"}`}
                >
                  <div className="flex items-baseline gap-2 mb-1">
                    <span className="text-xs font-semibold text-foreground">
                      {isMe ? user?.name : "Superadmin"}
                    </span>
                  ...
                  <div
                    className={`rounded-xl p-4 text-sm leading-relaxed ${isMe ? "bg-card border border-border/50 text-foreground" : "bg-primary/10 text-foreground border border-primary/20"}`}
                  >
                    {msg.body}
                  </div>
                </div>
